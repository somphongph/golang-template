...
import (
	...
	"golang-template/modules/examples/entities"
	"time"
...
type ExampleRepository interface {
	GetById(string) (entities.Example, error)
	Fetch(filter primitive.M, sort primitive.D, page, limit int) ([]entities.Example, error)
	Count(filter primitive.M) (int64, error)
	Add(*entities.Example) error
	Update(*entities.Example) error
	Delete(string) error
}
...
func InitExampleRepository(cfg *configs.MongoDB) *mongoDBStore {
...
func (s *mongoDBStore) GetById(in string) (entities.Example, error) {
...
func (s *mongoDBStore) Fetch(filter primitive.M, sort primitive.D, page, limit int) ([]entities.Example, error) {
	ctx := context.Background()
...
func (s *mongoDBStore) Add(p *entities.Example) error {
	ctx := context.Background()

	p.Status = "active"
	p.CreatedBy = getUserId()
...
func (s *mongoDBStore) Update(p *entities.Example) error {
	ctx := context.Background()
	update := bson.M{
		"$set": p,
	}
